...
| **Term Premium** | ACM affine term premia on the Fed GSW zero curve (or this app's own factor history), overlaid on the Federal Reserve Board's Kim-Wright term premia with correlation and mean-gap statistics, the Diebold-Li expectations split for comparison, a stacked yield decomposition (expectations + term premium + convexity) and Campbell-Shiller / Fama-Bliss tests. |
...
| `GET /api/term-premium?source=gsw\|treasury\|tips&start=&end=&maturities=2,5,10&factors=3&max_maturity=10&dns_method=var` | ACM term premia and decomposition, Kim-Wright benchmark with agreement statistics, Diebold-Li split, EH regressions |
...
| Fed Board Kim-Wright term premia (FRED `THREEFYTP1`-`THREEFYTP10`) | no | Published survey-anchored premia, overlaid as a benchmark; smoother than ACM by construction (no synthetic stand-in) |
